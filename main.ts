...
async function initDatabase() {
  ...
  try {
    ...
    try {
      ...
      // users 表添加咨询师相关字段
      await sql.unsafe(`
        ALTER TABLE users ADD COLUMN IF NOT EXISTS consultant_level VARCHAR(20) DEFAULT '件'middle';
      ...
      // products 表添加分类和单位字段
      await sql.unsafe(`
        ALTER TABLE products ADD COLUMN IF NOT EXISTS category VARCHAR(50) DEFAULT '件'other';
      ...
      try {
        ...
        logger.wan('乱码数据修复跳过: ' + fixError.message);
      }
